...
// ── Abrigos cadastrados (tabela dinâmica, sem zonas fixas) ──────────────────
$sqlAbrigos = "SELECT id, nome, localizacao, cep, descricao FROM abrigos ORDER BY nome ASC";
$stmtAbrigos = $pdo->prepare($sqlAbrigos);
$stmtAbrigos->execute();
$abrigosDb = $stmtAbrigos->fetchAll(PDO::FETCH_ASSOC);

// Indexado por id, para popular o objeto JS usado no modal de adoção
$abrigosPorId = [];
foreach ($abrigosDb as $ab) {
    $abrigosPorId[$ab['id']] = $ab;
}

// Monta a estrutura que vai para window.ABRIGOS_DATA (consumida pelo adocao.js)
$abrigosParaJs = [];
foreach ($abrigosDb as $ab) {
    $abrigosParaJs[$ab['id']] = [
        'nome'     => $ab['nome'],
        'endereco' => $ab['localizacao'],
        'cep'      => $ab['cep'],
    ];
}

// ── Animais disponíveis (com fotos agrupadas) ───────────────────────────────
$sqlAnimais = "SELECT
        a.id_animal, a.nome, a.idade, a.raca, a.especie, a.sexo, a.porte, a.peso,
        a.vacinado, a.castrado, a.abrigo, a.descricao, a.status_adocao,
...
// Converte 0/1/null do banco para texto exibível
function simNaoOuInformado($valor) {
...
<head>
  ...
  <!-- [ALTERADO #1] Adicionada Playfair Display para o texto "Pet Vida" no header -->
...
<body>
  ...
  <!-- ===== HEADER ===== -->
  <!-- [ALTERADO #1] Logo centralizada com texto "Pet Vida" em Playfair Display -->
  <header role="banner">
    <div class="header-inner">

      <!-- Logo + texto Pet Vida — bloco coeso centralizado -->
      <a href="index.php" class="logo" aria-label="Página inicial">
        <img src="img/logo_petvida.png" alt="Pet Vida" class="logo-img">
        <!-- [ALTERADO #1] Texto Pet Vida com Playfair Display -->
        <span class="logo-text">Pet <em>Vida</em></span>
      </a>

      <!-- Campo de busca centralizado na navbar -->
      ...
      <div class="nav-acoes" id="nav-acoes">
        <!-- [ALTERADO #4] Botão Favoritos agora é toggle inline (sem sidebar)
             [ALTERADO #7] Visual redesenhado: pill arredondada, sombra suave, hover com gradiente e badge com animação de "pop" -->
        <button class="btn-favoritos" id="btn-favoritos" aria-label="Mostrar apenas favoritos" aria-pressed="false">
          <!-- [ALTERADO #3] Ícone SVG moderno de coração -->
        ...
        <!-- [ALTERADO #5] Botão Pet Quiz: sem ícone, mais "gordinho", agora é um link para quiz.html -->
  ...
  <!-- [ALTERADO #2] Filtros completamente redesenhados — sem emojis, com ícones SVG e novos filtros Macho/Fêmea -->
  <div class="filtros-bar" id="filtros-bar" role="region" aria-label="Filtros de busca">
    <div class="filtros-bar-inner">
      ...
      <div class="filtros-grupo" role="group" aria-label="Filtrar por abrigo">
        <button data-filtro-abrigo="todos" class="filtro-pill filtro-ativo" id="filtro-todos">
          Todos os abrigos
        </button>
        <?php foreach ($abrigosDb as $ab): ?>
          <button data-filtro-abrigo="<?= $ab['id'] ?>" class="filtro-pill" id="filtro-abrigo-<?= $ab['id'] ?>">
      ...
      <!-- Filtros por tipo — [ALTERADO #2] sem emojis, ícones SVG -->
      <div class="filtros-grupo" role="group" aria-label="Filtrar por tipo de pet">
        <button data-filtro-tipo="todos" class="filtro-pill filtro-ativo" id="filtro-tipo-todos">
          Todos
        </button>
        <button data-filtro-tipo="cachorro" class="filtro-pill" id="filtro-cachorro">
          Cachorros
        </button>
        <button data-filtro-tipo="gato" class="filtro-pill" id="filtro-gato">
          Gatos
        </button>
      </div>
      ...
      <!-- [ALTERADO #2] Novos filtros: Macho e Fêmea -->
      <div class="filtros-grupo" role="group" aria-label="Filtrar por sexo">
        <button data-filtro-sexo="todos" class="filtro-pill filtro-pill--sexo filtro-ativo" id="filtro-sexo-todos">
          Ambos os sexos
        </button>
        <button data-filtro-sexo="macho" class="filtro-pill filtro-pill--sexo" id="filtro-macho">
          Macho
        </button>
        <button data-filtro-sexo="femea" class="filtro-pill filtro-pill--sexo" id="filtro-femea">
          Fêmea
        </button>
      </div>

      <!-- Separador + Limpar sempre juntos na mesma linha -->
      <div class="filtro-limpar-grupo">
        <div class="filtros-separador" aria-hidden="true"></div>
  ...
  <!-- ===== OVERLAY (para modal favoritos) — [ALTERADO #4] overlay não é mais usado para sidebar, mantido apenas por compatibilidade ===== -->
  <div class="overlay" id="overlay" role="presentation"></div>

  <!-- [ALTERADO #4] Modal de favoritos (sidebar) REMOVIDA da interface principal.
       O painel lateral foi mantido no HTML mas desativado visualmente — 
       o toggle agora filtra inline na grade principal. -->
      ...
      <div class="pets-container">
        ...
        <?php if (!empty($animais)): ?>
          <?php foreach ($animais as $animal): ?>
            <?php
                // Fotos do animal -> array de caminhos para o carrossel
                $fotos = [];
                if (!empty($animal['todas_fotos'])) {
                    foreach (explode(',', $animal['todas_fotos']) as $foto) {
                        $fotos[] = 'uploads/' . $foto;
                    }
                } else {
                ...
                $especieLower = strtolower($animal['especie']); // "cachorro" ou "gato"
                $abrigoId     = $animal['abrigo'];
                $abrigoNome   = $abrigosPorId[$abrigoId]['nome'] ?? 'Abrigo não informado';

                $vacinadoTxt = simNaoOuInformado($animal['vacinado']);
                $castradoTxt = simNaoOuInformado($animal['castrado']);
            ?>
            <article
                class="pet-card <?= htmlspecialchars($especieLower) ?>"
                data-id="<?= $animal['id_animal'] ?>"
                data-nome="<?= htmlspecialchars($animal['nome']) ?>"
                data-tipo="<?= htmlspecialchars($especieLower) ?>"
                ...
                data-vacinado="<?= $vacinadoTxt ?>"
                data-castrado="<?= $castradoTxt ?>"
                data-abrigo="<?= htmlspecialchars($abrigoId) ?>"
                data-status="<?= htmlspecialchars($animal['status_adocao']) ?>"
              ...
              <div class="card-img-wrapper">
                ...
                <img src="<?= htmlspecialchars($primeiraFoto) ?>" alt="<?= htmlspecialchars($animal['nome']) ?>, <?= htmlspecialchars($animal['raca']) ?>" loading="lazy">
                <?php if (strtolower($animal['status_adocao']) !== 'disponível'): ?>
                  <span class="badge urgente"><?= htmlspecialchars($animal['status_adocao']) ?></span>
                <?php endif; ?>
                <button class="btn-fav-card" aria-label="Favoritar <?= htmlspecialchars($animal['nome']) ?>" data-id="<?= $animal['id_animal'] ?>">
              ...
              <div class="card-body">
                <h3 class="pet-nome"><?= htmlspecialchars($animal['nome']) ?></h3>
                <p class="pet-info">
                  <span class="tag-tipo tag-<?= htmlspecialchars($especieLower) ?>"><?= htmlspecialchars($animal['especie']) ?></span>
                  · <?= htmlspecialchars($animal['porte']) ?> · <?= htmlspecialchars($animal['idade']) ?> ano(s)
                </p>
                <p class="pet-localizacao">
                  ...
                  <?= htmlspecialchars($abrigoNome) ?>
                </p>
                <button class="btn-detalhes" aria-label="Ver detalhes de <?= htmlspecialchars($animal['nome']) ?>">Ver Detalhes</button>
              </div>
            </article>
          <?php endforeach; ?>
        <?php else: ?>
          <p style="padding: 40px; text-align:center; width:100%;">Nenhum animal disponível para adoção no momento.</p>
        <?php endif; ?>
  ...
  <!-- ===== FOOTER ===== -->
  <!-- [ALTERADO #6] Links redirecionam para index.html; ícones de contato atualizados -->
  <footer role="contentinfo">
    <div class="footer-inner">
      <div class="footer-links">
        <h4>Institucional</h4>
        <!-- [ALTERADO #6] Todos redirecionam para index.html -->
      ...
      <!-- [ALTERADO #6] Seção de contato com WhatsApp, Email e Instagram -->
      <div class="footer-contato">
        ...
        <a href="tel:+554799756519" class="footer-contato-link">
          <!-- [ALTERADO #6] Ícone WhatsApp mantido -->
        ...
        <!-- [ALTERADO #6] Instagram adicionado explicitamente -->
        <a href="#" class="footer-contato-link">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
          @petvida.oficial
        </a>
        <!-- [ALTERADO #6] Ícones sociais: Email, Instagram, WhatsApp (Facebook removido) -->
    ...
    <div class="detalhes-container">
      ...
      <div class="info-col">
        <div class="info-pet">
          ...
          <div class="info-grid">
            <div class="info-item"><span class="info-label">Tipo</span><span class="info-valor" id="detalhes-tipo"></span></div>
            <div class="info-item"><span class="info-label">Raça</span><span class="info-valor" id="detalhes-raca"></span></div>
            <div class="info-item"><span class="info-label">Sexo</span><span class="info-valor" id="detalhes-sexo"></span></div>
            <div class="info-item"><span class="info-label">Idade</span><span class="info-valor" id="detalhes-idade"></span></div>
            <div class="info-item"><span class="info-label">Porte</span><span class="info-valor" id="detalhes-porte"></span></div>
            <div class="info-item"><span class="info-label">Peso</span><span class="info-valor" id="detalhes-peso"></span></div>
            <div class="info-item"><span class="info-label">Vacinado</span><span class="info-valor" id="detalhes-vacinado"></span></div>
            <div class="info-item"><span class="info-label">Castrado</span><span class="info-valor" id="detalhes-castrado"></span></div>
            <div class="info-item info-item--wide"><span class="info-label">Localização</span><span class="info-valor" id="detalhes-localizacao"></span></div>
          </div>
  ...
  <script>
    // Dados reais dos abrigos (tabela `abrigos`), usados pelo adocao.js no lugar
    // do objeto fixo que existia antes. Só temos nome, endereço (localizacao) e cep.
    window.ABRIGOS_DATA = <?= json_encode($abrigosParaJs, JSON_UNESCAPED_UNICODE) ?>;
  </script>
