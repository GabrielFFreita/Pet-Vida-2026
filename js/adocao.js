/* ============================================================
   ...
   // ── Dados dos abrigos ────────────────────────────────────────
   // [MESCLAGEM ADOCAO.PHP] Antes era um objeto fixo com 4 abrigos (telefone/horário
   // inventados). Agora vem direto do banco (tabela `abrigos`), injetado pelo PHP em
   // `window.ABRIGOS_DATA` (ver final de adocao.php). A tabela só tem nome, localização,
   // cep e descrição — não existe telefone nem horário cadastrados.
   const abrigos = window.ABRIGOS_DATA || {};
   ...
   function confirmarAdocao() {
     ...
     const enderecoAb = abrigo.endereco || '—';
     ...
     setTimeout(() => {
       qs('#info-final').innerHTML = `
         ...
         <p><strong>Endereço:</strong> ${enderecoAb}</p>
       `;
